...
def test(dataloader,net,DEVICE,MODE,Model_name,name_index,log_path,correct_on_test,correct_on_train,Model_custom_list, flag='test_set',output_flag = False):
    ...
    if output_flag == True:
        ...
        print(Model_name,MODE,name_index,macro_f1.item() ,micro_f1.item()  ,macro_p.item()  ,macro_r.item()  ,micro_p.item()  ,micro_r.item())
        # enable csv writing ___________________________________> look here
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path_rgb', type=str, default='./data/rgb', help='results path') #the path change, may diferent from the README. Please view github for detail.
    parser.add_argument('--path_ir', type=str, default='./data/thermal', help='results path')
    parser.add_argument('--index', type=int, default= 0, help='name index. any number is fine')
    parser.add_argument('--batch_size', type=int, default=64, help='batch_size')
    parser.add_argument('--lr', type=float, default=1e-4, help='learning rate, 1e-4 for small subset') #1e-4
    ...
    parser.add_argument('--model', type=str, default='Flame_two_stream', help='VGG16 and Flame_two_stream')
    ...
    # print(f"length {int(len(Dataset))}")
    #
    #
    ...
    ############################################################################################
        train_dataloader = DataLoader(dataset=train_set, batch_size=BATCH_SIZE, shuffle=True, pin_memory = True, num_workers = 4, persistent_workers=True, drop_last = True)
        test_dataloader = DataLoader(dataset=val_set, batch_size=BATCH_SIZE, shuffle=True, pin_memory = True, num_workers = 4, persistent_workers=True, drop_last = True)
        ...
        for index in range(EPOCH):
            ...
            if (index + 1) == EPOCH:
                ...
                if current_accuracy > max_accuracy:
                    ...
                    torch.save(net, f'saved_model/{file_name} batch={BATCH_SIZE} acc={current_accuracy}.pkl')
                break
    ...
    #$important$ 
    torch.save(net, f'saved_model/{file_name} final batch={BATCH_SIZE} acc={acc}.pkl')
